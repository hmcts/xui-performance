    ...
    * Click on 'Children and respondents'
    ...
    group("XUI_PRL_C100_550_ChildrenAndRespondents") {
      ...
      .exec(http("XUI_PRL_C100_550_020_ChildrenAndRespondents")
        ...
        .header("accept", "application/vnd.uk.gov.hmcts.ccd-data-store-api.ui-start-event-trigger.v2+json;charset=UTF-8")
        .check(jsonPath("$.event_token").saveAs("event_token"))
        .check(jsonPath("$.case_fields[0].value[0].value.respondentFullName").saveAs("respondentName"))
        .check(jsonPath("$.case_fields[0].value[0].value.childFullName").saveAs("childName"))
        .check(jsonPath("$.case_fields[0].value[0].id").saveAs("respondentId"))
        .check(jsonPath("$.case_fields[0].value[0].value.respondentId").saveAs("respondentNameId"))
        .check(jsonPath("$.case_fields[0].value[0].value.childId").saveAs("childId"))
